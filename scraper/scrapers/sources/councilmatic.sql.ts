export const getSQLForBills = (chunkSize: number, offset: number) => {
  return `SELECT
  b.*,
  json_extract(bv.extras, '$.summary') AS bill_summary,
  (
    SELECT
      json_group_array(
        json_object(
...
FROM
  bill AS b
  LEFT JOIN billversion AS bv ON b.id = bv.bill_id
WHERE
  json_extract(b.extras, '$.routine') = false
